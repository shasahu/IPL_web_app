...
question = "For any year, you need to show top 10 bowlers(rank them according to their total wickets taken)."
description = "Analysis show rank of bowler according to total wickets taken by them in particular IPL. There is option to choose IPL year. Bar chart showing top 10 bowler is also shown which is categorize according to team. It also contain 4's  & 6's hitted on him, overs spelled, maiden over spelled, runs hitted on him, wickets taken by him, dot balls, wides , noballs bowl by him and economy in that IPL."
...
def scorecard_bowl(match_id,teams_name):
...
def app():
    with open("log/{}.csv".format(st.session_state.curUserID),"a") as f:
            write = writer(f)
            write.writerow([str(datetime.now()),"ANALYSIS","3 2/{}".format(question)])
            f.close()
    ...
    cols1[1].selectbox("IPL YEAR",years,index=0,key="bowlque2selectbox1")

    if(st.session_state.bowlque2selectbox1 != "none"):
        choosed_year = st.session_state.bowlque2selectbox1
        ...
        for x in mat_id_teams:
            ...
            temp_df = scorecard_bowl(x[0],x[1].split(','))
            overall_df = pd.concat([overall_df,temp_df],ignore_index=True)
    
        grp_overall = overall_df.groupby("Bowler")
    
        overall_df1 = grp_overall.sum()
        overall_df1["Economy"] = [(overall_df1.loc[x[0],"Runs"]/overall_df1.loc[x[0],"Overs"]) for x in grp_overall]
        overall_df1["Team"] = [x[1].iloc[0,-1] for x in grp_overall]
    
        overall_df1 = overall_df1.sort_values(by="Wickets",ascending=False)
